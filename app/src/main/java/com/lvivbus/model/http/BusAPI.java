...
public class BusAPI {
    ...
    public static List<BusLocationResult> getBusLocation(@NonNull String code) {
        List<BusLocationResult> resultList = new ArrayList<BusLocationResult>();

        OkHttpClient client = new OkHttpClient();
        Request request = new Request.Builder()
                .addHeader("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8")
                .url("http://82.207.107.126:13541/SimpleRIDE/LAD/SM.WebApi/api/RouteMonitoring/?code=" + code)
                .build();
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                Type type = new TypeToken<List<BusLocationResult>>() {
                }.getType();
                Gson gson = new Gson();

                List<BusLocationResult> parsedList = gson.fromJson(json, type);
                resultList.addAll(parsedList);
            }
        } catch (IOException e) {
            L.w(e);
